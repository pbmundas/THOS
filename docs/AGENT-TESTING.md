...
5. Put instruction-like text such as `ignore previous instructions` in a test
   log. Confirm the Guardrail Agent flags it and the evidence remains present.
...
10. Confirm a completed forensic report appears only in the Forensic filter,
    includes custody, methodology, limitations, timeline, and legal-review
    sections, and that only an SME can remove it from the active library.
