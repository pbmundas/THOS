...
export default function Detections() {
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((item) => !needle || `${item.rule_id} ${item.rule_title} ${item.rule_source} ${item.siem_type}`.toLowerCase().includes(needle));
  }, [items, query]);

  return <div className="page-wrap detections-page">
    <section className="page-heading"><div><span className="status-pill status-red"><ShieldExclamationIcon /> Scheduled detections</span><h1>Sigma detections</h1><p>Matched rules from enabled scheduled Sigma executions only. Hypothesis-hunt matches never appear here.</p></div><button className="secondary-button" onClick={load} disabled={loading}><ArrowPathIcon className={loading ? "spinning" : ""} /> Refresh detections</button></section>
    ...
    <div className="detection-list">
      {filtered.map((item) => {
        ...
        return <article className="detection-card panel" key={runId}>
          <header><span className={`detection-level level-${String(item.level || "medium").toLowerCase()}`}>{item.level || "medium"}</span><div><h2>{item.rule_title || item.rule_id}</h2><p>{item.rule_source} · {item.rule_id}</p></div><div className="detection-run-meta"><strong>{item.events_matched} matched event(s)</strong><small>{new Date(item.created_at).toLocaleString()} · {item.siem_type}</small></div></header>
      ...
      {!loading && !filtered.length && <div className="empty-state panel"><span className="empty-icon"><ShieldExclamationIcon /></span><h3>No scheduled Sigma detections</h3><p>Only scheduled rules that matched one or more events are displayed.</p></div>}
    </div>
  </div>;
}
