    ...
    <div className="detection-list">
      {filtered.map((item) => {
        ...
        return <article id={`detection-${runId}`} className={`detection-card panel ${String(focusId) === runId ? "focused" : ""}`} key={runId}>
          ...
          {liveAnalysis && analysisExpanded === runId && <section className="detection-ai-analysis" id={`detection-analysis-${runId}`}><div><strong><SparklesIcon /> Detection Analysis Agent</strong><span>{liveAnalysis.cached ? "Cached model analysis" : "Local model analysis"} · {liveAnalysis.confidence || "medium"} confidence</span></div><ol>{(liveAnalysis.analysis_lines || []).map((line, index) => <li key={`${runId}-analysis-${index}`}>{displaySource(line)}</li>)}</ol><small>{liveAnalysis.detection_uid || item.detection_uid} · {liveAnalysis.agent?.model_name || "Evidence-bounded analysis"} · {liveAnalysis.generated_at ? new Date(liveAnalysis.generated_at).toLocaleString() : ""}</small></section>}
